...
def lighting(components, modes, numberComponents, numberModes):
    ...
    lightingTitle = tk.Label(lightingGUI, text = "SUNLIGHT/SHADE REQUIREMENTS")
    ...
    lightingFinish = tk.Button(lightingGUI, text = "FINISH", command = lightingGUI.destroy)
    lightingBack = tk.Button(lightingGUI, text = "BACK", command = back) #Constraints_GUI.constraint(components, modes, numberComponents, numberModes))
    ...
    lightingFinish.grid(row = numberModes + 4, column = 0, columnspan = 4, sticky = tk.W + tk.E)
    lightingBack.grid(row = numberModes + 5, column = 0, columnspan = 4, sticky = tk.W + tk.E)
    ...
    for (modeName, mode), lightingRow in zip(modes.items(), range(numberModes)):
        ...
        for text, index in zip(LIGHTING_OPTIONS, range(len(LIGHTING_OPTIONS))):
            ...
            lightingChecks[index] = tk.Checkbutton(lightingGUI, text=text, variable=varLighting[index])
