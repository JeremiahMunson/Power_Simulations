...
def switching(components, modes, numberComponents, numberModes):
    ...
    switchOptions = tk.Label(switchGUI, text = "When/Why Does Mission Enters Mode")
    switchRank = tk.Label(switchGUI, text = "Mode Rank")
    switchGap = tk.Label(switchGUI, text = "\t")
    switchContinue = tk.Button(switchGUI, text = "CONTIUE", command = switchGUI.destroy)
    switchBack = tk.Button(switchGUI, text = "BACK", command = back)
    ...
    switchContinue.grid(row = 4 + numberModes, column = 0, columnspan = 6, sticky = tk.W + tk.E)
    #switchBack.grid(row = 5 + numberModes, column = 0, columnspan = 6, sticky = tk.W + tk.E)
    ...
    for (modeName, mode), switchModesRows in zip(modes.items(), range(numberModes)):
        ...
        varSwitch[modeName].set(mode.printRequirement()) # initialize
        switchMode = tk.Label(switchGUI, text = modeName) # Doesn't need to be list because it doesn't have a variable and gets placed a few lines below
        varRanking[modeName] = (tk.IntVar())
        varRanking[modeName].set(mode.printRank())
        modeRanking[modeName] = (tk.Entry(switchGUI, textvariable = varRanking[modeName]))
    
        for text, i in zip(OPTIONS, range(len(OPTIONS))):
            switchButtons = tk.Radiobutton(switchGUI, text=text, variable=varSwitch[modeName], value=OPTIONS[text])
            switchButtons.grid(row = 2+switchModesRows, column = i+1)
        switchMode.grid(row = (2 + switchModesRows), column = 0)
        modeRanking[modeName].grid(row = (2+switchModesRows), column = len(OPTIONS)+1)
    ...
    if (first):
        {mode.assignRequirement(varSwitch[modeName].get(), varRanking[modeName].get()) for modeName, mode in modes.items()}
        return Constraints_GUI.constraint(components, modes, numberComponents, numberModes)
