...
def size(components, modes):
    ...
    sizeContinue = tk.Button(sizeGUI, text = "CONTINUE", command = sizeGUI.destroy)

    numComponentsVariable = tk.StringVar()
    numComponentsVariable.set(3)
    numModesVariable = tk.StringVar()
    numModesVariable.set(4)
    numComponentsInput = tk.Entry(sizeGUI, textvariable = numComponentsVariable)
    numModesInput = tk.Entry(sizeGUI, textvariable = numModesVariable)

    sizeTitle.grid(row = 0, column = 0, columnspan = 2, sticky = tk.W + tk.E)
    ...
    sizeContinue.grid(row = 4, column = 0, columnspan = 2, sticky = tk.W + tk.E)

    sizeGUI.mainloop()

    return Components_GUI.comp(components, modes, int( numComponentsVariable.get()), int(numModesVariable.get()))